...
func main() {
	content, _ := ioutil.ReadFile("internal/adapter/postgres/post_repo.go")
	s := string(content)

	oldStart := "func (r *PostRepo) ListFeed(ctx context.Context, cursor string, limit int) ([]domain.Post, string, error) {"
	oldEnd := "\treturn posts, nextCursor, nil\n}"

	startIdx := strings.Index(s, oldStart)
	endIdx := strings.Index(s[startIdx:], oldEnd) + startIdx + len(oldEnd)

	oldFunc := s[startIdx:endIdx]
	...
	// Add strconv if not present
	if !strings.Contains(s, `"strconv"`) {
		s = strings.Replace(s, `"fmt"`, "\"fmt\"\n\t\"strconv\"", 1)
	}

	ioutil.WriteFile("internal/adapter/postgres/post_repo.go", []byte(s), 0644)
}
